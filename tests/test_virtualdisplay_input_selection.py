...
class VirtualDisplayInputSelectionTests(unittest.TestCase):
    ...
    def test_setup_tracks_original_sunshine_audio_sink_without_overwriting_it(self) -> None:
        ...
        original_record_stock_units = manager._record_stock_units
        ...
        try:
            ...
            manager._stop_stock_units = lambda: None
            ...
            manager._record_stock_units = original_record_stock_units
            ...
            manager._stop_stock_units = original_stop_stock_units
    ...
    def test_managed_sunshine_templates_do_not_force_global_pulse_sink(self) -> None:
        ...
        sway_unit = units[Path(state["paths"]["sway_unit"])]
        audio_guard_unit = units[Path(state["paths"]["audio_guard_unit"])]
        sunshine_unit = units[Path(state["paths"]["sunshine_unit"])]
        ...
        self.assertNotIn("Environment=PULSE_SINK=", sway_unit)
        self.assertIn(f"After={manager.SUNSHINE_UNIT}", audio_guard_unit)
        self.assertIn(f"PartOf={manager.SUNSHINE_UNIT}", audio_guard_unit)
        self.assertIn(f"Wants={manager.AUDIO_GUARD_UNIT}", sunshine_unit)
        self.assertNotIn("Environment=PULSE_SINK=", sunshine_unit)
